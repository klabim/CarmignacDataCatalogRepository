...
describe('Component Tests', () => {
  describe('OrderedSource Management Update Component', () => {
    ...
    describe('ngOnInit', () => {
      it('Should call linkedSource query and add missing value', () => {
        const orderedSource: IOrderedSource = { id: 456 };
        const linkedSource: ISource = { id: 51799 };
        orderedSource.linkedSource = linkedSource;

        const linkedSourceCollection: ISource[] = [{ id: 27031 }];
        spyOn(sourceService, 'query').and.returnValue(of(new HttpResponse({ body: linkedSourceCollection })));
        const expectedCollection: ISource[] = [linkedSource, ...linkedSourceCollection];
        ...
        expect(sourceService.addSourceToCollectionIfMissing).toHaveBeenCalledWith(linkedSourceCollection, linkedSource);
        expect(comp.linkedSourcesCollection).toEqual(expectedCollection);
      });
      ...
      it('Should update editForm', () => {
        const orderedSource: IOrderedSource = { id: 456 };
        const linkedSource: ISource = { id: 7058 };
        orderedSource.linkedSource = linkedSource;
        ...
        expect(comp.linkedSourcesCollection).toContain(linkedSource);
